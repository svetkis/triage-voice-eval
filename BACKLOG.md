...
### [x] #10 Async-guards
Готово: в раннере `if asyncio.iscoroutine(out): out = await out`. Работает и для async, и для sync без изменения базового контракта. Тест через AsyncGuard с `asyncio.sleep(0)`.
...
### [x] #14 Переименовать `UsageLogger` → `UsageTracker`
Готово: новый модуль `usage_tracker.py` с классом `UsageTracker`. `usage_logger.py` — shim с `__getattr__`, который возвращает `UsageTracker` + `DeprecationWarning`. Тесты обновлены, README обновлён.
